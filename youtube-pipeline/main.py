...
def _setup_logging() -> None:
    ...
    rotating = logging.handlers.RotatingFileHandler(LOG_FILE, maxBytes=2_000_000, backupCount=5)
...
def run_single_video(
    *,
    ...
    """Fetch, summarize, and post one specific video URL, bypassing channel scan and lookback."""
    logger = logging.getLogger("pipeline")
    logger.info("Single-video mode: %s (dry_run=%s)", video_url, dry_run)

    # Extract video ID from URL for ChannelFetcher's individual metadata fetch.
    import re as _re
    ...
    if db.has_been_posted(video_id):
        logger.info("Video %s has already been posted — skipping", video_id)
        return 0
    ...
    published_at = fetcher._fetch_upload_date(video_id)
    if published_at is None:
        logger.error("Could not fetch metadata for %s", video_id)
        return 1

    # Resolve channel name from config if the URL is from a configured channel,
    # otherwise fall back to the uploader name from yt-dlp.
    import subprocess as _sp, json as _json

    try:
        meta_raw = _sp.run(
            ["yt-dlp", "--dump-json", "--no-warnings", "--no-playlist", "--skip-download", video_url],
            capture_output=True, text=True, timeout=30, check=False,
        )
        meta = _json.loads(meta_raw.stdout) if meta_raw.returncode == 0 else {}
    except Exception:
        meta = {}

    channel_name = meta.get("uploader") or meta.get("channel") or "Unknown"
    ...
    transcript_fetcher = TranscriptFetcher(max_chars=int(pipeline_cfg["max_transcript_chars"]))
    ...
    if dry_run:
        logger.info("[dry-run] would summarize %s (%d chars) and post to Discord", video_id, len(transcript))
        return 0
    ...
    if summary is None:
        logger.error("Summarization failed for %s", video_id)
        return 1

    poster = DiscordPoster(webhook_url)
    message_id = poster.post_summary(summary)
    if not message_id:
        logger.error("Discord post failed for %s", video_id)
        return 1
    ...
    logger.info("Posted %s -> Discord %s", video_id, message_id)
    db.close()
    return 0
...
def run(
    *,
    ...
    logger = logging.getLogger("pipeline")
    logger.info("Pipeline starting (dry_run=%s, channel=%s)", dry_run, channel_filter or "*")
    ...
    try:
        ...
        since = _resolve_lookback_since(db, int(pipeline_cfg.get("lookback_hours", 26)))
        ...
        videos_new = videos_found - videos_already_posted
        summary_line = (
            f"Run complete: {len(channels)} channel(s) checked, "
            f"{videos_found} video(s) found "
            f"({videos_already_posted} already posted, {videos_new} new), "
        ...
        if not dry_run and webhook_url:
            if not success:
                alert = f"⚠️ YouTube pipeline FAILED\n```\n{(_fatal_error or 'unknown error')[:1800]}\n```"
                try:
                    DiscordPoster(webhook_url).post_run_summary(alert)
                except Exception as e:
                    logger.warning("Failure alert post failed: %s", e)
            elif videos_posted or videos_skipped_no_transcript or videos_skipped_summary_failed:
                try:
...
def main() -> int:
    ...
    parser.add_argument("--db", type=Path, default=DEFAULT_DB, help="Path to SQLite state DB")
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Process a single video URL directly, bypassing channel scan and lookback window",
    )
