...
class DiscordPoster:
    ...
    def _build_embed(self, summary: Summary) -> dict:
        ...
        fields = []
        trades_value = self._format_trades(summary.proposed_trades)
        if trades_value:
            fields.append({"name": "📈 Proposed Trades", "value": trades_value, "inline": False})
        if takeaways_value:
            fields.append({"name": "Key Takeaways", "value": takeaways_value, "inline": False})
        if fields:
            embed["fields"] = fields
        return embed

    @staticmethod
    def _format_trades(trades: list[str]) -> str:
        if not trades:
            return ""
        bullets = [f"• {t}" for t in trades]
        joined = "\n".join(bullets)
