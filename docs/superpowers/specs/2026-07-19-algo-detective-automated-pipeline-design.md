...
- **PDF-era recap posts (2025-09-01 through 2025-12-29).** Verified by fetching raw HTML: the structured `trades-table` HTML format only exists from `results_boring_puts_2026_01_05` onward — every recap post from 2025-09-01 through 2025-12-29 (18 weeks) instead links to a downloadable PDF trade log and mentions tickers only in unstructured prose (e.g. "Cash Secured Puts (CSP) on DELL, NVDA*, UAL, GOOG*", no per-ticker dates). Parsing PDFs is materially different engineering (different library, different failure modes, much less structurally reliable) and is out of scope here. Automated backfill via this plan reaches back to **2026-01-05 only** (~30 weeks as of 2026-07-13). The existing hand-curated `prime_tickers.csv` remains the historical record for the Sep-Dec 2025 period; a future plan could add PDF parsing if that gap matters later.
...
- **No `trades-table` element found on the post** (pre-2026-01-05 PDF-era post, or a genuine zero-trades week): treated as a *valid* zero-result outcome, not an error — `fetch_recap_trades()` returns `[]`, the slug **is** recorded in `detective_scraped_posts` (0 trades found), and it's never retried. This is the expected, common case for the whole PDF-era range and must not be logged as a warning or retried forever.
- **`trades-table` element present but a row fails to parse** (unexpected column count, unparseable ticker/date cell — a genuine HTML-structure-change scenario): caught per-post inside `label_sync`, logged at `WARNING` with the slug and exception. The slug is **not** recorded in `detective_scraped_posts`, so it's retried automatically every night — no backoff, no dead-lettering. Matches the codebase's existing simplicity convention; nightly retries of a cheap HTTP fetch cost nothing.
